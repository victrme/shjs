...
sh_languages["log"] = [
	[
		...
		[/^\[[A-Za-z]{3}[ \t][A-Za-z]{3}[ \t]{1,2}[\d]{1,2}[ \t](?=[\d]{2}:[\d]{2}:[\d]{2})/g, "sh_date", 8, 1],
	...
	[
		...
		[/\b(?:OPTIONS|GET|HEAD|POST|PUT|DELETE|TRACE|CONNECT|PROPFIND|MKCOL|COPY|MOVE|LOCK|UNLOCK)\b/g, "sh_webmethod", 7, 1],
	],
	[[/[^ \t]+/g, "sh_string", -2]],
	[
		[/$/g, null, -2],
		[/\b[\d]{2}:[\d]{2}:[\d]{2}\b/g, "sh_time", -1],
		[/[\d]{4}\]|\[[A-Za-z]{3}[ \t][A-Za-z]{3}[ \t]{1,2}[\d]{1,2}[ \t](?=[\d]{2}:[\d]{2}:[\d]{2})/g, "sh_date", -1],
		[/\[error\]/g, "sh_string", -1],
